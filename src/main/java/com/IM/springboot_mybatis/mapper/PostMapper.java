...
public interface PostMapper {
    ...
    @Results(id = "postMap",value = {
            ...
            @Result(property = "isLiked",column = "{askId = askId,postId = postId}",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.isLiked")),
            @Result(property = "likeCount",column = "postId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getPostsLikeCount")),
            @Result(property = "forwardCount",column = "postId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getPostsForwardNum")),
            @Result(property = "commentCount",column = "postId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getPostsCommentNum")),
            ...
            @Result(property = "isStar",column = "{askId = askId,postId = postId}",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.isStar")),
            @Result(property = "forwardName",column = "forwardId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getNameByPostId")),
            @Result(property = "forwardText",column = "forwardId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getTextByPostId")),
            @Result(property = "forwardImageUrl",column = "forwardId",
                    one = @One(select = "com.IM.springboot_mybatis.mapper.PostMapper.getImageByPostId")),
            })
    ...
    //获取动态的点赞数
    @Select("select count(*) as likeCount from postliked where postId = #{postId}")
    List<Integer> getPostsLikeCount(Integer postId);
    //获取动态的转发数
    @Select("select count(*) as forwardCount from post where forwardId = #{postId}")
    ...
    //给动态点赞
    @Insert("insert into postliked (userId,postId) values(#{userId},#{postId})")
    Integer likePost(Integer userId ,Integer postId);
    //取消动态点赞
    @Delete("DELETE FROM postliked WHERE userId = #{userId} and postId = #{postId}")
    ...
    //是否给动态点赞了
    @Select("select count(*) from postliked where userId = #{askId} and postId = #{postId}")

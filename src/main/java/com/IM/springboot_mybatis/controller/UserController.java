...
import com.IM.springboot_mybatis.service.impl.UserServiceImpl;
import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.IM.springboot_mybatis.entity.Updates;
import com.IM.springboot_mybatis.entity.User;
import com.IM.springboot_mybatis.service.MailService;
...
import com.IM.springboot_mybatis.utils.RandomCodeUtil;
...
public class UserController {
    ...
    public JsonResult<User> logIn(User user,HttpSession session) {
        User req = userService.logIn(user);
        if(req!=null){
            session.setAttribute(Constant.USER_TOKEN, req.getUserId());
        }
        JsonResult<User> json;
        if(req!=null) json = new JsonResult<>(req, "1", "验证成功");
        else json = new JsonResult<>(null, "0", "验证失败");
        return json;
    }
    ...
    public int updatePosition(int userId,double latitude, double longitude){
        return userService.updatePosition(userId,latitude,longitude);
    }
    ...
    public JsonResult<List<User>> getLikedUser(Integer askId,Integer postId,Integer page) {
        ...
        json = !res.isEmpty()
                ? new JsonResult<>(res, "1", "查询成功", pageInfo.getPages())
                : new JsonResult<>(res, "0", "查询失败", pageInfo.getPages());
        return json;
    }
    ...
    public  JsonResult<Integer> isExistTheUsername(String username){
        JsonResult<Integer> json;
        Integer res = userService.isUsernameExist(username);
        json = new JsonResult<>(Integer.toString(res),"");
        return json;
    }

    @RequestMapping("/getNearUser")
    public JsonResult<List<User>> getNearUser(int userId){
        List<User> res = userService.getNearUser(userId);
        return new JsonResult<>(res, "1", "成功");
    }
}

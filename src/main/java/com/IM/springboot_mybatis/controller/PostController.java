...
import com.IM.springboot_mybatis.service.PostService;
import com.IM.springboot_mybatis.utils.JsonResult;
import com.IM.springboot_mybatis.utils.PostHotUtil;
...
public class PostController {
    ...
    public JsonResult<List<Post> > getPostsById(Integer askId,Integer userId,Integer page) {
        ...
        if(!res.isEmpty()){
            ...
            json =new JsonResult<>(res, "0", "动态数量为0", pageInfo.getPages());
        }
        return json;
    }
    @RequestMapping("/getPostsOrderByHot")
    ...
    @RequestMapping("/getPostsOrderByDate")
    public JsonResult<List<Post> > getAllPostsByDate(Integer userId,Integer page) {
        ...
        PageInfo<Post> pageInfo = new PageInfo<Post>(res);
        JsonResult<List<Post> > json;
        if(!res.isEmpty()) json =new JsonResult<List<Post> >(res, "1", "获取到动态",pageInfo.getPages());
        else json =new JsonResult<>(res, "0", "动态数量为0", pageInfo.getPages());
        return json;
    }
    ...
    @RequestMapping("/getFavouritePosts")
    ...
    @RequestMapping("/addFavouritePost")
    ...
    @RequestMapping("/removeFavouritePost")
